...
import { eventsTable, showsTable, venuesTable, showSeatsTable, seatLayoutsTable, bookingsTable, bookingSeatsTable, waitlistTable } from "@workspace/db/schema";
import { eq, and, inArray } from "drizzle-orm";
import { requireAuth } from "../middlewares/auth";
import { CreateBookingBody, CreateSeatHoldBody, JoinWaitlistBody } from "@workspace/api-zod";
const router = Router();
function makeQr(reference) {
...
router.post("/shows/:id/holds", requireAuth, async (req, res) => {
    const showId = parseInt(req.params.id);
    ...
    try {
        const holdResult = await db.transaction(async (tx) => {
            ...
            const seatsToHold = await tx.select()
                .from(showSeatsTable)
                .where(and(eq(showSeatsTable.showId, showId), inArray(showSeatsTable.id, seatIds)));
...
router.post("/bookings", requireAuth, async (req, res) => {
    ...
    const heldSeats = await db.select().from(showSeatsTable)
        .where(and(eq(showSeatsTable.heldBy, req.user.id), eq(showSeatsTable.status, "held")
    ...
    await sendEmail(req.user.email, "Your Paradox Ticket is Confirmed", `<p>Thank you for your booking.</p><p>Your unique QR Ticket Code: <strong>${qrUrl}</strong></p><img src="${qrUrl}" alt="Ticket QR" />`);
    res.status(201).json({
        id: `b-${Date.now()}`,
        reference,
...
// @ts-nocheck
router.post("/admin/venues", requireAuth, async (req, res) => {
...
router.post("/organiser/events", requireAuth, async (req, res) => {
    if (req.user?.role !== "organiser" && req.user?.role !== "admin")
