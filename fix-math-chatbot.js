...
code = code.replace(/\{\[\['Gross revenue', '[^\]]+'\], \['Tickets moved', '684'\], \['Sell-through', '87.6%'\], \['Avg\. ticket', '[^\]]+'\]\]/g, 
  "{[['Gross revenue', '?17,10,000'], ['Tickets moved', '684'], ['Sell-through', '87.6%'], ['Avg. ticket', '?2,500']]");

code = code.replace(/<h2 className="display-font mt-4 text-5xl">Nocturne for a City<\/h2><p className="mt-5 text-sm text-foreground\/55">112 of 120 seats booked [^<]+<\/p>/g,
  '<h2 className="display-font mt-4 text-5xl">Arijit Singh Live</h2><p className="mt-5 text-sm text-foreground/55">112 of 120 seats booked · ?2,80,000 gross</p>');
...
function Chatbot() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      {open && (
        <div className="mb-4 w-80 rounded-2xl border border-foreground/15 bg-background shadow-2xl overflow-hidden">
          ...
          <div className="p-3 bg-card border-t border-foreground/10 flex flex-wrap gap-2">
            {faqs.map((faq, i) => (
              <button key={i} onClick={() => handleFaq(faq)} className="text-[10px] rounded-full border border-foreground/20 px-3 py-1.5 hover:bg-foreground/5 transition-colors">
                {faq.q}
              </button>
            ))}
      ...
      <button onClick={() => setOpen(!open)} className="h-14 w-14 rounded-full bg-accent text-accent-foreground shadow-lg grid place-items-center hover:scale-105 transition-transform">
...
// Inject <Chatbot /> right before <Toaster />
